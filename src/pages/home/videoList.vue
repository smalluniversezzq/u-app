<template>
    <div class="videoList  aa" v-if="data">
      ...
      <!--<div v-if="$bizId === 4 && isShow" style="margin-bottom: 30px;">
        <p class="ways">影视代理</p>
        <div class="progressBox progressBox1">
          <x-progress :percent="( vipData.buy_sum_money/ 1) * 100 > 100 ? 100 : ( vipData.buy_sum_money / 1) * 100" :show-cancel="false"></x-progress>
          <p v-if="true">{{vipData.need_self_buy}}元</p>
        </div>
        <p class="present">自购佣金在{{vipData.need_self_buy}}块钱以上：</p>
        <div class="progressBox progressBox1">
          <x-progress :percent="(vipData.invite_count / vipData.need_invite_count) * 100 > 100 ? 100 : (vipData.invite_count / vipData.need_invite_count) * 100" :show-cancel="false"></x-progress>
          <p v-if="true">{{vipData.need_invite_count}}人</p>
        </div>
        <p class="present">当前有效直属粉丝：{{vipData.invite_count}}人</p>
      </div>-->
    </div>
  ...
  methods: {
    ...
    getInfo () {
      this.$http.post('/amoy/movie/up-conditions', {}, true).then(res => {
        ...
        // this.vipData = {"need_self_buy":"1.00","need_invite_count": 10,"buy_sum_money": 0,"invite_count": 5,"finish": 1}
