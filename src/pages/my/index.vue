<template>
  <div id="my" class="tbkMy">
    ...
    <div class="moneysBox">
      <!--<span @click="jumpToVideo">视频</span>-->
      ...
      <div class="headBox" :style="{marginTop: settingH + 1 + 'rem'}">
        <div class="top">
          ...
          <div class="profitBox">
            ...
            <!-- @click="jumpTo('/operatorcenter')" -->
            <div class="profit"  @click="toshouyi(data.lv)">
              <p v-if="data && data.total_income">￥{{data.total_income}}</p>
              <p v-else> 0.00</p>
              <p>累积收益</p>
            </div>
            <div class="profit" @click="jumpTo('/shoppingnum')">
              <p v-if="data && data.credit5">{{data.credit5}}</p>
              <p v-else> 0.00</p>
              <p>购物通积分</p>
            </div>
          </div>
        ...
        <div class="bot">
          <div><span>可提现余额</span><span v-if="data && data.credit3">￥{{data.credit3}}元</span><span v-else>0.00</span></div>
          <div @click="jumpTo('/withdraw')"><span class="look">申请提现</span></div>
          <!-- profit -->
        </div>
      </div>
      <!-- <div  @click="jumpTo('/operatorcenter')">operatorcenter</div> -->
      <!-- <div  @click="jumpTo('/guide')">guide</div> -->
      <template v-for="item of myContent">
        <edit-slide v-if="item.name === 'edit-slide' && slide"  :data="item.params"  :types="2" :states='data.lv' ref="editSlide" :price='data.credit1' :key="item.id" ></edit-slide>
        <component v-if="item.name !== 'edit-slide' && item.name !== 'edit-my-header'" :is="item.name" :states='data.lv'  :data="item.params"  :price='data.credit1' v-bind:types="2" :key="item.id"></component>
      </template>
    </div>
    </mescroll-vue>
  </div>
