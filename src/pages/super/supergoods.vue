<template>
  <div id="goods" ref="goods" v-if="other">
    ...
    <div id="box" ref="box">
      <div class="with" @click="withs(other)" :style="{top:(h) + 'rem'}">同款 </div>
      ...
      <div class="goodsInst">
        ...
        <div class="priceBox">
          ...
          <div class="profit"  :style="{background: 'linear-gradient('+$store.state.global.theme.deg+'deg,' +$store.state.global.theme.mainColor+','+$store.state.global.theme.secondColor+')'}">预估积分{{other.commission_credit}}</div>
        </div>
        <div class="update" v-if="other.level_commission_money && (other.level_commission_money !== other.commission_money) && lvstate ==='普通会员'"   @click="jumpTo('/partnerships?state=' + lvstate)">
          <p :style="{color: $store.state.global.theme.mainColor}">现在升级合伙人，最高获得{{other.level_commission_money}}佣金</p><span :style="{background: 'linear-gradient('+$store.state.global.theme.deg+'deg,' +$store.state.global.theme.mainColor+','+$store.state.global.theme.secondColor+')'}">立即升级</span>
        </div>
        <div class="update" v-if="other.level_commission_money && (other.level_commission_money !== other.commission_money) && lvstate ==='合伙人'"   @click="jumpTo('/partnership?state=' + lvstate)">
        ...
        <div class="update" v-if="other.level_commission_money && (other.level_commission_money !== other.commission_money) && lvstate ==='运营商'"   @click="jumpTo('/partnership?state=' + lvstate)">
          <p :style="{color: $store.state.global.theme.mainColor}">运营商，最高获得{{other.level_commission_money}}佣金</p><span :style="{background: 'linear-gradient('+$store.state.global.theme.deg+'deg,' +$store.state.global.theme.mainColor+','+$store.state.global.theme.secondColor+')'}">点击查看</span>
        </div>
        <div class="coupon" @click="tobuy" v-if="other.coupon_money">
          ...
          <p class="money" v-if="Number(other.coupon_money) >= 99" style="font-size: 0.8rem"><span style="font-size: 0.3rem">元</span>{{Number(other.coupon_money)}}</p>
    ...
    <div class="footer">
      ...
      <div class="collect" @click="share"  v-if="$bizId !== 27">
    ...
    <div class="goodsBarrage" :class="{showBarrage: showBarrage}">
...
export default {
  ...
  methods: {
    ...
    share () {
      if (!utils.storage.get('token')) {
        ...
        if (window.api && api.pageParam.backToWin) {
          ...
          api.execScript({
            name: 'main',
            script: `vueObj.jumpTo("/wechatLogin")`
          })
          setTimeout(() => {
            // 关闭本frame和所属的window
    ...
    //       }
    //     })
    //   }else if(this.states === '运营商'){
    //     this.$router.push({
    //       path:"/operatorcenter",
    //       query:{
    //         state:"operatorcenter"
    //       }
    //     })
    //   }
    // },
    ...
    getInfo () {
      if (this.type === 0 || this.type === 1) {
        ...
        console.log('/amoy/taobao/goods-detail')
  ...
  mounted () {
     this.lvstate = utils.storage.get('userInfo').lv;
     console.log(this.lvstate,"lvstate")
    if (!this.other.shopName) {
      this.getInfo()
    }
    this.showBarrage = false
    ...
    document.getElementsByClassName('footer')[0].style.paddingBottom = (window.api && api.safeArea.bottom || 0) + 'px'
    let size = (document.documentElement.clientWidth / 7.5)
    document.getElementsByClassName('goods_back_top_box')[0].style.bottom = ((window.api && api.safeArea.bottom || 0) / size) + 1.4 + 'rem'
    let that = this
    if (window.api) {
