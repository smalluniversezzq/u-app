<template>
    <div class="order">
      <tab :line-width="2" class="my-or-team" custom-bar-width=".4rem" :active-color="$store.state.global.theme.mainColor" default-color="#333" v-model="type" v-if="!isSearchResult">
        <tab-item @on-item-click="reset(1)">30日内已到账收入</tab-item>
        <tab-item @on-item-click="reset(0)">30日内未到账收入</tab-item>
      </tab>
      ...
      <div class="listBox" :style="{height: isSearchResult? 'calc(100% - 1.6rem)' :'calc(100% - 88px - 1.6rem)'}">
        <mescroll-vue ref="mescroll" :down="mescrollDown" :up="mescrollUp" @init="mescrollInit">
          <div class="high"></div>
          <div class="list" v-for="(item, index) of data" :key="index">
            <div class="top">
...
export default {
  ...
  methods: {
    ...
    reset (type) {
      if(type == 0){
        console.log("30天内未到账金额列表")
        //30天内未到账金额列表  amoy/shop/unsettlement
        this.weidaozhang();
      }else if(type == 1){
        console.log("30日内已到账收入")

        //30日内已到账收入 amoy/shop/settlement
        this.yidaozhang();
      }
    },
    weidaozhang(){
      this.$http.post('/amoy/shop/unsettlement', {
        }, false, true).then((res) => {
        if (res.code === 0) {
    ...
    yidaozhang(){
      this.$http.post('/amoy/shop/settlement', {
        }, false, true).then((res) => {
        if (res.code === 0) {
          console.log(res.data)
           this.data = res.data;
