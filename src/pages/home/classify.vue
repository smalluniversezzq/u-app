      ...
      <div class="list" :class="{over: showMore}"  :style="{top: (0.88 + statusH) + 'rem'}">
        <div class="rank vux-1px-t">
          <tab class="tab" :line-width="0" v-model="nowIndex" :active-color="$store.state.global.theme.mainColor" default-color="#333">
            ...
            <tab-item class="tab_price"  @on-item-click="showsScreening" >
               <p><span>价格</span></p>
            </tab-item>
          ...
          <div v-show='showsScreenings' class="sprice">
            <div class="topto_box">
              <div class="high_price price_btn" @click="tabon(1)" :style="{color: sort===1?'#eb662d':'black'}">价格升序</div>
              <div class="low_price price_btn" @click='tabon(2)'  :style="{color: sort===2?'#eb662d':'black'}">价格降序</div>
            </div>
...
export default {
  ...
  methods: {
    ...
    upCallback (page, mescroll) {
      ...
      //if  如果是按照常规查询  else if  如果是按照价格区间查询 具体传什么参数  可以定义一个状态 pricenum = true 然后查询  ps:err
    ...
    //价格筛选
    tabon (val) {
      if (val === 1 && this.sort !== 1) {
        this.sort = 1;
        this.reset();
      } else if(val === 2 && this.sort !== 2){
        this.sort = 2;
        this.reset();
      } else if(val === 88){
         this.sort = 0;
