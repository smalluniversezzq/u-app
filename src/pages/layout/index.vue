<template>
  <view-box ref="viewBox" id="layout" :body-padding-top="$route.meta.isShowHead || $route.meta.isShowSearch ? '.88rem' : '0'" :body-padding-bottom="$route.meta.isShowTab ? h : '0'">
    ...
    <operatorAlert  v-show="$store.state.global.setshowoperator === true"></operatorAlert>
    <!--<tabbar slot="bottom" class="tabbar" ref="tabbar" id="tabbar" v-if="$store.state.global.footer" v-show="$route.meta.isShowTab" :style="{background:$store.state.global.footer.bgColor}">-->
      ...
      <!--</tabbar-item>-->
    <!--</tabbar>-->
    <div class="tabbarBox" id="tabbar1" ref="tabbar" v-if="$store.state.global.mallFooter" v-show="$route.meta.isShopShowTab && ($store.state.global.firstNav !== '/home' || $isMallShop)" :style="{background:$store.state.global.mallFooter.bgColor}">
...
import operatorAlert from '../../components/operatorAlert'
export default {
  name: 'index',
  data () {
    return {
